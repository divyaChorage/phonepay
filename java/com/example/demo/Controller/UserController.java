...
public class UserController {
	...
	@DeleteMapping("unlinkBankAccount/{bankId}")
	public int deleteLinkedAcc(@PathVariable int bankId) {
	    System.out.println(bankId + " delete");

	    LinkedAccount user = accountRepo.findByBankid(bankId);
	    if (user == null) {
	        return 0;
	    }
	    try {
	        user.setIsdeleted(1); // Mark as deleted
	        accountRepo.save(user); // Save the updated account with isdeleted = 1
	        return 1;
	    } catch (Exception e) {
	        e.printStackTrace();
	    }
